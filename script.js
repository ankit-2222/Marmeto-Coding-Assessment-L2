...
function toggleProduct(productId) {
    ...
    const existingItemIndex = bundleItems.findIndex(item => item.id === productId);
    const button = document.querySelector(`[onclick="toggleProduct(${productId})"]`);

    if (existingItemIndex > -1) {
        bundleItems.splice(existingItemIndex, 1);
...
function renderBundleItems() {
  ...
  for (let i = 0; i < maxSlots; i++) {
    ...
    if (item) {
      ...
      slotsHtml += `<div class="bundle-slot empty"></div>`;
    }
  }

  bundleItemsContainer.innerHTML = slotsHtml;
}
...
function completeBundle() {
    ...
    console.log('Bundle completed successfully:', bundleDetails);
}
